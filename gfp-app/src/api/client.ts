...
export const Api = {
  ...
  savePhotoMeal: (payload: {
    ...
    log_date?: string;
    confidence?: any;
  }) => api(EP.photoMeal, { method: 'POST', body: { ...payload, mode: 'log' } }),
  ...
  /** GET /companion/places - Google Places nearby search. The server takes
   *  lat/lng plus kind ('gym' or 'store'); a free-text query is not supported. */
  places: (opts?: { lat?: number; lng?: number; kind?: 'gym' | 'store' }) => {
    const p = new URLSearchParams();
    if (opts?.lat != null) p.set('lat', String(opts.lat));
    if (opts?.lng != null) p.set('lng', String(opts.lng));
    if (opts?.kind) p.set('kind', opts.kind);
    const qs = p.toString();
    return api<any>(qs ? `${EP.places}?${qs}` : EP.places);
  },
